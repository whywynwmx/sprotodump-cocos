...
local function dump_class(class_info, stream, deep, parent_cls)
  ...
  if sproto_type then
    ...
    stream:write(sformat("function _encode_%s(self: %s, st: SprotoStream) {", class_name, class_name), deep)
...
local function parse_protocol(class, stream, package)
  ...
  local class_name = _gen_protocol_classname(package)
  stream:write("export class "..class_name.." extends PbBase {")
...
local function parse_ast2type(ast, package, name)
  ...
  stream:write(header)
  stream:write([[import { SprotoTypeDeserialize } from "../../core/sproto/SprotoTypeDeserialize";
import { SprotoCore, ALL_DICT } from "../../core/sproto/SprotoCore";
import { SprotoStream } from "../../core/sproto/SprotoStream";]])
...
local function parse_ast2protocol(ast, package)
  ...
  stream:write(header)
  stream:write([[import { PbBase } from "../../core/sproto/PbBase"]])
  stream:write(using)
...
local function parse_ast2all(ast, package, name)
  ...
  import { SprotoCore, ALL_DICT } from "../../core/sproto/SprotoCore";
  import { SprotoStream } from "../../core/sproto/SprotoStream";
  import { PbBase } from "../../core/sproto/PbBase";]])
  -- stream:write([[// source: ]]..(name or "input").."\n")
  stream:write(using)
